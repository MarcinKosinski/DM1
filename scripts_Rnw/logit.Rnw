...
return(sum(Pop)/nrow(se_wyb))
}
# klaster obliczeniowy
require(parallel)
cl <- makeCluster(detectCores())
@
<<klustrowanie,results='hide', cache=TRUE>>=
clusterEvalQ(cl, library("foreign"))
clusterEvalQ(cl,se <- read.arff("http://archive.ics.uci.edu/ml/machine-learning-databases/00266/seismic-bumps.arff"))
clusterEvalQ(cl,se <- se[,-c(14:16)])
clusterEvalQ(cl,se_wyb <- se[,-c(9,15)])
@
<<obliczeniaklastrowe>>=
parLapply(cl, 1, kros)
stopCluster(cl)
