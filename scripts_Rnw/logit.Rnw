...
Z racji na du¿y zbiór danych, aby przyspieszyæ kroswalidacjê, dziêki pakietowi \texttt{parallel} stworzono odpowiedni klaster obliczeniowy, rozdzielaj±cy obliczenia na tyle w±tków ile jest na procesorze rdzeni.
...
for(i in 1:10){
   ...
   mod <- glm(class~., data=dane[-co,], family="binomial")
   pr <- predict(mod,type="response",newdata=se_wyb[co,])
   Predi <- ifelse(pr>0.5,1,0)   
   t <- tabela(Predi,dane[co,]$class)
   pro[i] <- procent(t)
   czu[i] <- czulosc(t)
   pre[i] <- precyzja(t)
...
plot(rlas)
@
Dzia³a powy¿ej - dodaæ sensowne komentarze :) !
<<>>=
...
roc(Pred.bic, Test$class)
@

Wspó³czynnik AUC wynosi:

<<echo=FALSE>>=
auc(Pred.bic, Test$class)
@
...
logit_ost$roc <- list(Pred.bic, Test$class) 
logit_ost$auc <- auc(Pred.bic, Test$class)
@
